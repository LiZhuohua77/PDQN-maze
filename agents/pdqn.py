...
class ParamActor(nn.Module):
    ...
    def __init__(self, state_size, action_size, action_parameter_size, hidden_layers, activation="relu"):
        ...
        self.action_parameter_size = action_parameter_size
        #print("Action parameter size: ", self.action_parameter_size)
        self.activation = activation
        self.layers = nn.ModuleList()
    ...
    def _optimize_td_loss(self):
        ...
        for name, param in self.actor.named_parameters():
            if param.grad is not None:
                print(f'Q Network Parameter: {name}, Gradient: {param.grad}')
        if self.clip_grad > 0:
        ...
        # 打印actor参数梯度
        for name, param in self.actor_param.named_parameters():
            if param.grad is not None:
                print(f'Actor Parameter: {name}, Gradient: {param.grad}')
        if self.clip_grad > 0:
